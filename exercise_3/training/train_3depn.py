...
def train(model, train_dataloader, val_dataloader, device, config):
    ...
    pure_l1_loss = torch.nn.L1Loss()

    smooth_l1_loss.to(device)
    pure_l1_loss.to(device)
    ...
    for epoch in range(config['max_epochs']):
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            loss = smooth_l1_loss(reconstruction, target) + pure_l1_loss(reconstruction, target)
            loss.backward()
            optimizer.step()
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in val_dataloader:
                    ...
                    loss_val += pure_l1_loss(reconstruction, target).item() + smooth_l1_loss(reconstruction, target).item()
