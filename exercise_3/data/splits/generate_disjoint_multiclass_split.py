...
with open(f'/workspace/project/dsdf/exercise_3/data/splits/sofa_bed_disjoint/overfit.txt', 'w') as f:
    for item in all_items[:num_overfit]:
        f.write(f'{item} {combined_dict[item]}\n')
    

with open(f'/workspace/project/dsdf/exercise_3/data/splits/sofa_bed_disjoint/train.txt', 'w') as f:
   for item in all_items[num_overfit:num_overfit+num_train]:
        f.write(f'{item} {combined_dict[item]}\n')


with open(f'/workspace/project/dsdf/exercise_3/data/splits/sofa_bed_disjoint/val.txt', 'w') as f:
...
for item in all_items:
    if item in sofa_items:
        shutil.copytree(f'/workspace/project/data/sofa/{item}', f'/workspace/project/data/sofa_bed_disjoint/{item}')
    elif item in bed_items:
        shutil.copytree(f'/workspace/project/data/bed/{item}', f'/workspace/project/data/sofa_bed_disjoint/{item}')